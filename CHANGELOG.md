...
- validate session tokens against `^[A-Za-z0-9_-]+$` before interpolating them
  into URL path segments in `status`, `cancel`, and `result`, preventing a
  path-traversal via crafted tokens (GHSA-rjfw-pvxw-2prx)
